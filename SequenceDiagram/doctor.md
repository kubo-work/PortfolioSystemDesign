...
sequenceDiagram
    ...
    alt 新規診察作成時
        ...
        患者情報画面->患者情報編集画面:患者情報編集画面にアクセス
        患者情報編集画面->>API:患者の情報を取得
        API->>データベース:アクセス
        データベース-->>API:結果を返す
        API-->>患者情報編集画面:患者の登録情報が格納されている編集画面を表示

        患者情報編集画面->>API:データを保存する
        API->>データベース:アクセス
        データベース-->>API:結果を返す
        alt 保存成功時
